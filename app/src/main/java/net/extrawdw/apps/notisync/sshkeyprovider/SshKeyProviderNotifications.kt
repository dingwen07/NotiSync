...
class SshKeyProviderNotificationPresenter(
    ...
    fun post(
        ...
        val keyName = when (stored.kind) {
            ...
            SshProviderRequestKind.IMPORT -> stored.importRequest?.suggestedName
                ...
                ?: context.getString(R.string.ssh_key_provider_imported_key_default)
        }.take(MAX_CONTEXT_CHARS)
        val title = when (stored.kind) {
            SshProviderRequestKind.SIGN -> destination?.let {
                context.getString(R.string.ssh_key_provider_notification_sign_title_with_destination, it)
            } ?: context.getString(R.string.ssh_key_provider_notification_sign_title)
            SshProviderRequestKind.IMPORT -> context.getString(
                R.string.ssh_key_provider_notification_import_title,
                keyName,
            )
        }
        val content = when (stored.kind) {
            SshProviderRequestKind.SIGN -> context.getString(
                R.string.ssh_key_provider_notification_sign_content,
                safeRequesterName,
                keyName,
            )
            SshProviderRequestKind.IMPORT -> context.getString(
                R.string.ssh_key_provider_notification_import_content,
                safeRequesterName,
                keyName,
            )
        }
        val expandedDetails = when (stored.kind) {
            SshProviderRequestKind.SIGN -> {
                listOfNotNull(
                    destination?.let {
                        context.getString(R.string.ssh_key_provider_notification_destination, it)
                    },
                    process?.take(MAX_CONTEXT_CHARS)?.let {
                        context.getString(R.string.ssh_key_provider_notification_process, it)
                    },
                    context.getString(R.string.ssh_key_provider_notification_key, keyName),
                )
            }
            SshProviderRequestKind.IMPORT -> listOfNotNull(
                when (stored.history.importSourceType ?: stored.importRequest?.sourceType) {
                    SshImportSourceType.AGENT_IDENTITY ->
                        context.getString(R.string.ssh_key_provider_import_source_agent)
                    SshImportSourceType.PRIVATE_KEY_FILE ->
                        context.getString(R.string.ssh_key_provider_import_source_file)
                    null -> null
                }?.let { context.getString(R.string.ssh_key_provider_notification_source, it) },
            )
        }
        ...
        val review = PendingIntent.getActivity(
            ...
            SshKeyProviderReviewActivity.intent(context, stored.requestId),
        ...
        val automaticReview = PendingIntent.getActivity(
            context,
            notificationId(stored.requestId),
            SshKeyProviderReviewActivity.autoOpenIntent(context, stored.requestId),
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE,
            requestPagePendingIntentOptions(),
        )
        val approve = PendingIntent.getActivity(
            context,
            notificationId(stored.requestId),
            SshKeyProviderReviewActivity.approveIntent(context, stored.requestId),
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE,
        )
        val reject = PendingIntent.getBroadcast(
            context,
            notificationId(stored.requestId),
            SshKeyProviderActionReceiver.rejectIntent(context, stored.requestId),
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE,
        )
        ...
        val publicVersion = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentText(context.getString(R.string.ssh_key_provider_notification_public_content))
    ...
    fun postAutoApproved(stored: StoredSshProviderRequest, requesterName: String): Boolean {
        ...
        val content = context.getString(
            R.string.ssh_key_provider_notification_auto_approved_content,
            safeRequesterName,
            keyName,
        )
        val expandedText = buildList {
            add(content)
            destination?.let { add(context.getString(R.string.ssh_key_provider_notification_destination, it)) }
            process?.let { add(context.getString(R.string.ssh_key_provider_notification_process, it)) }
            add(context.getString(R.string.ssh_key_provider_notification_key, keyName))
            add(context.getString(R.string.ssh_key_provider_notification_request, stored.requestId.take(8)))
        }.joinToString("\n")
        ...
        val forget = PendingIntent.getBroadcast(
            context,
            notificationId(stored.requestId),
            SshKeyProviderActionReceiver.forgetAuthorizationIntent(
                context,
                stored.requestId,
                authorizationId,
            ),
        ...
        val publicVersion = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_terminal_notification)
            .setContentTitle(context.getString(R.string.ssh_agent_name))
            .setContentText(context.getString(R.string.ssh_key_provider_notification_auto_approved_public_content))
            .setCategory(NotificationCompat.CATEGORY_STATUS)
            .setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
            .build()
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_terminal_notification)
            .setContentTitle(context.getString(R.string.ssh_key_provider_notification_auto_approved_title))
            .setContentText(content)
            .setStyle(NotificationCompat.BigTextStyle().bigText(expandedText))
            .setSubText(context.getString(R.string.ssh_agent_name))
            .setCategory(NotificationCompat.CATEGORY_STATUS)
            ...
            .setContentIntent(review)
            .setAutoCancel(true)
            .addAction(0, context.getString(R.string.ssh_key_provider_remembered_delete_confirm), forget)
            .build()
        NotificationManagerCompat.from(context).notify(notificationId(stored.requestId), notification)
        return true
    }
    ...
    fun dismiss(requestId: String) {
        PendingIntent.getActivity(
            ...
            SshKeyProviderReviewActivity.autoOpenIntent(context, requestId),
    ...
    private fun ensureChannel() {
        context.getSystemService(NotificationManager::class.java).createNotificationChannel(
            NotificationChannel(
                CHANNEL_ID,
                context.getString(R.string.ssh_key_provider_notification_channel),
                NotificationManager.IMPORTANCE_HIGH,
            ).apply {
                description = context.getString(R.string.ssh_key_provider_notification_channel_description)
