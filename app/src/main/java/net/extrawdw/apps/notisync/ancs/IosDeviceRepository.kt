...
class IosDeviceRepository {
    ...
    fun setStatus(status: AncsStatus) { _status.value = status }
    ...
    fun setDeviceName(name: String?) { if (!name.isNullOrBlank()) _deviceName.value = name }

    /** Drop the device name (no device connected), so a later session never shows a previous iPhone's name. */
    fun clearDeviceName() { _deviceName.value = null }
}
