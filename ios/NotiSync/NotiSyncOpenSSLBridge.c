...
NSScreenTLSListener *NSScreenTLSListenerCreate(
    ...
    *port = 0;
    ...
    ERR_clear_error();
    listener->context = SSL_CTX_new(TLS_server_method());
    if (listener->context == NULL ||
        !SSL_CTX_set_min_proto_version(listener->context, TLS1_3_VERSION) ||
        !SSL_CTX_set_max_proto_version(listener->context, TLS1_3_VERSION) ||
        !SSL_CTX_set_ciphersuites(
            listener->context,
            "TLS_AES_128_GCM_SHA256:TLS_CHACHA20_POLY1305_SHA256") ||
        !SSL_CTX_set1_groups_list(listener->context, "X25519")) {
        NSScreenTLSSetError(errorBuffer, errorBufferLength, "could not configure TLS 1.3");
        NSScreenTLSListenerDestroy(listener);
        return NULL;
    }
    SSL_CTX_set_app_data(listener->context, listener);
    SSL_CTX_set_verify(listener->context, SSL_VERIFY_NONE, NULL);
    SSL_CTX_set_session_cache_mode(listener->context, SSL_SESS_CACHE_OFF);
    SSL_CTX_set_num_tickets(listener->context, 0);
    SSL_CTX_set_max_early_data(listener->context, 0);
    SSL_CTX_set_options(listener->context, SSL_OP_NO_TICKET | SSL_OP_NO_RENEGOTIATION);
    SSL_CTX_set_mode(listener->context, SSL_MODE_AUTO_RETRY);
    SSL_CTX_set_psk_server_callback(listener->context, NSScreenTLSPSKCallback);
    SSL_CTX_set_alpn_select_cb(listener->context, NSScreenTLSALPNCallback, NULL);
...
int NSScreenTLSListenerAccept(
    ...
    *connection = NULL;
    ...
    if (SSL_version(ssl) != TLS1_3_VERSION || !supportedCipher ||
...
int NSScreenTLSConnectionReadExactly(
    ...
    if (!NSScreenTLSSetSocketTimeout(socketDescriptor, timeoutMilliseconds)) {
...
void NSScreenTLSConnectionClose(NSScreenTLSConnection *connection) {
    ...
    NSScreenTLSCloseSocket(&connection->socketDescriptor);
}
